    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as response:
            ...
            if expected_key in res_data:
                ...
                elif expected_key == "results" and len(res_data["results"]) > 0:
                    top_score = res_data["results"][0].get("relevance_score", res_data["results"][0].get("score", "N/A"))
                    print(f"  ✅ {name} is ONLINE. Top relevance score: {top_score}")
                else:
...
def test_services_online():
    """Pytest entrypoint to verify all local service endpoints are responsive."""

...
## 3. Validation Implementation Phases (The "How")

- **Phase 1: Audit (MANDATORY)**: Before writing any tasks, output a section named `## Damage Report`. For each modified function, explicitly state:
    - **Original contract**: what it returned and how it behaved
    - **Modified contract**: what changed
    - **Verdict**: `REVERT`, `KEEP` (genuine bug fix), or `PARTIAL` (revert with surgical exception)

    Use this report to justify every task you generate. Do not write any tasks until the Damage Report is complete.

- **Phase 2: Reversion & Surgical Fixes**: The Editor strips out hallucinated or damaging changes from the source code, applying only Architect-approved safety checks (e.g., handling NAs, preventing state leakage).

- **Phase 3: Test Correction**: The Editor updates the `testthat` scripts to correctly mock environments and assert against the true, original source code contracts.

---

## 4. Atomic Task List Requirements

> Architect: For each task, provide the "Tight Description" the Editor needs to implement the reversion or fix. Separate tasks for source code fixes and test script fixes.

### [Task ID: 001] - [Task Title]

- **Target File**: `R/target_source_file.R` OR `tests/testthat/test-target_file.R`

- **Scope Variables** _(comma-separated list of every function, variable, or block this task touches)_:

- **Completeness Check**: Confirm your Syntax Example explicitly handles ALL Scope Variables listed above. A reversion or fix that addresses only one of multiple affected functions is a task failure. A NULL placeholder or ellipsis for any Scope Variable is non-compliant.

- **Essential Elements**: (Brief comma-separated list of the functions or behaviors to be reverted/fixed)

- **Tight Description**: Provide precise logic for the reversion, the specific minimal bug fix to apply, or the exact test assertion/mock data update required.

- **Syntax Example**: Provide verbatim 2 lines of existing code immediately before and after each change. No ellipses (`...`) to represent skipped code. A placeholder for any Scope Variable is non-compliant.
...
## 5. Validation Summary (list format)

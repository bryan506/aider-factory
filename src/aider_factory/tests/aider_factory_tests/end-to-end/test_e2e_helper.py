...
with tempfile.TemporaryDirectory() as tmp_boot:
    ...
    assert res_boot.returncode == 0, f"bootstrap failed: {res_boot.stderr}"
    af_dir = os.path.join(tmp_boot, ".aider_factory")
    assert os.path.isdir(af_dir), "bootstrap must create .aider_factory/"
    yaml_files = [f for f in os.listdir(af_dir) if f.startswith(".env_") and f.endswith(".yml")]
    assert len(yaml_files) >= 1, f"Expected .env_*.yml in .aider_factory/, got: {yaml_files}"
    with open(os.path.join(af_dir, yaml_files[0]), "r") as f:
        content = f.read()
    assert "test_runner:" in content, "Generated YAML must contain test_runner key"
    assert "pytest" in content, "Pytest framework must be detected from pytest.ini"
    assert "working_directory:" in content, "Generated YAML must set working_directory"
...
with tempfile.TemporaryDirectory() as tmp_repo_target:
    with tempfile.TemporaryDirectory() as tmp_cwd:
        ...
        assert res_repo.returncode == 0, f"bootstrap --repo failed: {res_repo.stderr}"
        af_target = os.path.join(tmp_repo_target, ".aider_factory")
        assert os.path.isdir(af_target), "--repo must scaffold in target, not cwd"
...
with tempfile.TemporaryDirectory() as tmp_parent:
    ...
    assert res_create.returncode == 0, f"--repo nonexistent must not crash: {res_create.stderr}"
    assert os.path.isdir(os.path.join(nonexistent_target, ".aider_factory")), \
